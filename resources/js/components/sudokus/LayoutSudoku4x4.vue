...
const props = defineProps({
    ...
    sound_item_4: {type: String, required: true},
    ...
    selector_5: {type: Array},
    selector_6: {type: Array},
})
...
function showIndexSquare() {
    ...
    setTimeout(function () {
        ...
        if (dato3.value.type === 'color') {
            imageIndexSquare.value = false
            indexSquare.value = `border-black border-2 h-24 w-24 bg-${dato4.value.value} flex justify-center items-center`;
            contentIndexSquare.value = null
        }
        if (dato3.value.type === 'item') {
            imageIndexSquare.value = false
...
const paintString = (id, numIndex) => {
    ...
    document.getElementById(id).classList.remove(`bg-${props.color_4}`)
    ...
    if (paintType === 'color') {
        ...
        } else if (colorSelected.value === `bg-${props.color_4}`) {
            // console.log('3')
        ...
        if (content.value === dato1.value.value) {
            ...
            sequenceNumber.value = '3'
        } else if (content.value === dato4.value.value) {
            sequenceNumber.value = '4'
    ...
    const select_fila = (filaNum, color, error) => {
        ...
        if (filaNum === 4) {
            y = props.sudoku_size + props.sudoku_size + props.sudoku_size + 1
            x = props.sudoku_size + props.sudoku_size + props.sudoku_size + 1
        }
    ...
    const nice = (focusBox, selector, audioNice, audioOk) => {
        ...
        if (props.interactive_array_solution[focusBox] == 1) {
            audioItem = `${props.sound_item_1}`;
        } else if (props.interactive_array_solution[focusBox] == 2) {
            audioItem = `${props.sound_item_2}`;
        } else if (props.interactive_array_solution[focusBox] == 3) {
            audioItem = `${props.sound_item_3}`;
        } else if (props.interactive_array_solution[focusBox] == 4) {
            audioItem = `${props.sound_item_4}`;
        }

        const audio1 = new Audio(audioNice);
        const audio2 = new Audio(audioItem);
        const audio3 = new Audio(audioOk);
        ...
        if (selector[1] === 0) {
            ...
            audioCol = `${props.asset_audio}/voz1/sudokus/fyc/yprimeracolumna.m4a`
        } else if (selector[1] === 2) {
            audioCol = `${props.asset_audio}/voz1/sudokus/fyc/ysegundacol.m4a`
        } else if (selector[1] === 3) {
            audioCol = `${props.asset_audio}/voz1/sudokus/fyc/yterceracolumna.m4a`
        } else if (selector[1] === 4) {
            audioCol = `${props.asset_audio}/voz1/sudokus/fyc/ycuartacolumna.m4a`
        }

        if (selector[0] === 0) {
            audioCol = null
            if (selector[1] === 1) {
                audioCol = `${props.asset_audio}/voz1/sudokus/fyc/primeracol.m4a`
            } else if (selector[1] === 2) {
                audioCol = `${props.asset_audio}/voz1/sudokus/fyc/segundacol.m4a`
            } else if (selector[1] === 3) {
                audioCol = `${props.asset_audio}/voz1/sudokus/fyc/terceracol.m4a`
            }
        } else if (selector[0] === 1) {
            audioRow = `${props.asset_audio}/voz1/sudokus/fyc/primerafila.m4a`
        } else if (selector[0] === 2) {
            audioRow = `${props.asset_audio}/voz1/sudokus/fyc/segundafila.m4a`
        } else if (selector[0] === 3) {
            audioRow = `${props.asset_audio}/voz1/sudokus/fyc/tercerafila.m4a`
        } else if (selector[0] === 4) {
            audioRow = `${props.asset_audio}/voz1/sudokus/fyc/cuartafila.m4a`
        }

        if (audioRow && audioCol) {
            onlyOne = false
            audio4.src = audioRow
            audio5.src = audioCol
            audio6.src = `${props.asset_audio}/voz1/sudokus/${props.palette_text}/sigamossiguienteposicion.m4a`
        } else if (audioRow && !audioCol) {
            audio4.src = audioRow
            audio5.src = `${props.asset_audio}/voz1/sudokus/${props.palette_text}/sigamossiguienteposicion.m4a`
        } else {
            audio4.src = audioCol
            audio5.src = `${props.asset_audio}/voz1/sudokus/${props.palette_text}/sigamossiguienteposicion.m4a`
        }
        ...
        audio3.onended = function () {
            ...
            for (let i = 0; i < props.sudoku_size; i++) {
                if (selector[0] === 0) {
                    select_col(selector[1], 'bg-yellow-500', false)
                    setTimeout(function () {
                        select_fila(selector[0], 'bg-yellow-500', false)
                    }, 2000)
                } else {
                    select_fila(selector[0], 'bg-yellow-500', false)
                    setTimeout(function () {
            ...
            setTimeout(function () {
                if (nextSelectBox.value >= props.interactive_array.length && onlyOne === true) {
                    setTimeout(function () {
                        win()
                    }, 800)
                } else {
    ...
    const badNice = (focusBox, selector, audioError) => {
        ...
        if (sequenceNumber.value === '1') {
            ...
            audioItem = `${props.sound_item_3}`;
        } else if (sequenceNumber.value === '4') {
            audioItem = `${props.sound_item_4}`;
        }

        const audio1 = new Audio(audioItem);
        const audio2 = new Audio(audioError);
        ...
        audio2.onended = function () {
            ...
            for (let i = 0; i < props.sudoku_size; i++) {
                if (selector[0] === 0) {
                    select_col(selector[1], 'opacity-75', true)
                    setTimeout(function () {
                        select_fila(selector[0], 'opacity-75', true)
                    }, 2000)
                } else {
                    select_fila(selector[0], 'opacity-75', true)
                    setTimeout(function () {
    ...
    if (interactiveMode.value === true) {
        ...
        if (props.interactive_array_solution[nextSelectBox.value] == sequenceNumber.value) {
            ...
            console.log('BOX ACTUAL:', nextSelectBox.value)

            if (nextSelectBox.value === 0 && props.ok_audio_1) {

                nice(nextSelectBox.value,
                    props.selector_1,
                    `${props.asset_audio}/voz1/nice/muybien.m4a`,
                    `${props.ok_audio_1}`)
            }
            ...
            if (nextSelectBox.value === 1 && props.ok_audio_2) {
                ...
                nice(nextSelectBox.value,
                    props.selector_2,
                    `${props.asset_audio}/voz1/nice/correcto.m4a`,
                    `${props.ok_audio_2}`,)
            }
            ...
            console.log('Fallo :', {
                'nextSelectBox': nextSelectBox.value,
                'sequenceNumber': sequenceNumber.value
            })

            if (nextSelectBox.value === 0 && props.error_audio_1) {

                badNice(nextSelectBox.value,
                    props.selector_1,
                    props.error_audio_1)
            }
...
const prepareSudoku = () => {
    ...
    for (let caja = 1; caja <= 16; caja++) {
        ...
        if (props[`caja_${caja}`] == 4) {

            if (dato4.value.type == 'image') {
                imageUpdate(caja, dato4)
            }

            if (dato4.value.type === 'color') {
                colorUpdate(caja, dato4)
            }

            if (dato4.value.type == 'item') {
                itemUpdate(caja, dato4)
            }
        }
    }
};
                                ...
                                <div class="flex justify-center col-span-2">
                                    <div>
                                        <div class="grid grid-cols-2 gap-5 mt-5">
                                            ...
                                            <div class="flex justify-center">
                                                <button v-if="dato4.type === 'image'"
                                                        @click="selectItem(dato4.type, dato4.value)"
                                                        class="flex justify-center items-center ">
                                                    <div class="bg-gray-300 p-4 shadow-md rounded-lg">
                                                        <img :src="dato4.value" width="40" alt="">
                                                    </div>
                                                </button>

                                                <button v-if="dato4.type === 'color'"
                                                        @click="selectItem(dato4.type, dato4.value)"
                                                        class="rounded-md p-2 shadow-md bg-gray-300">
                                                    <svg :class="props.icon_4" xmlns="http://www.w3.org/2000/svg"
                                                         width="50"
                                                         height="50" viewBox="0 0 24 24">
                                                        <path
                                                            d="M0 21.398c5.504.456 3.533-5.392 8.626-5.445l2.206 1.841c.549 6.645-7.579 8.127-10.832 3.604zm16.878-8.538c1.713-2.687 7.016-11.698 7.016-11.698.423-.747-.515-1.528-1.17-.976 0 0-7.887 6.857-10.213 9.03-1.838 1.719-1.846 2.504-2.441 5.336l2.016 1.681c2.67-1.098 3.439-1.248 4.792-3.373z"/>
                                                ...
                                                <button v-if="dato4.type === 'item'"
                                                        @click="selectItem(dato4.type, dato4.value)"
                                                        class="rounded-md py-3 px-5 shadow-md bg-gray-300">
                                                    <span class="font-bold text-4xl">{{ dato4.value }}</span>
                                                </button>
                                            </div>

                                            <button @click="selectItem(null, null)"
                                                    class="relative rounded-md p-2 shadow-md bg-gray-300  flex justify-center select-none col-span-2">
                                                <div class="relative">
